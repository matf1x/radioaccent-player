...
export const newSession = new Promise(async (resolve, reject) => {
  ...
  try {
    ...
    if (!("streamIndex" in localStorage))
      localStorage.setItem("streamIndex", 0)
    if (!("stream" in localStorage))
      localStorage.setItem(
        "stream",
        audio.streams[localStorage.getItem("streamIndex")].url
      );
    if ("volume" in localStorage)
      elements.player.volume.slider.value = localStorage.getItem("volume") * 100
    ...
    player.volume = localStorage.getItem("volume")
      ...
      : audio.volume;
    player.src = localStorage.getItem("stream")
      ? localStorage.getItem("stream")
      : audio.streams[0].url

    // Check if the audio can play
    player.addEventListener("canplay", () =>
      resolve({ player, elements, audio })
    )
  } catch (e) {
    // Throw a error
